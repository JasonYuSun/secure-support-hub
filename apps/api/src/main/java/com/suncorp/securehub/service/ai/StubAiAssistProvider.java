...
public class StubAiAssistProvider implements AiAssistProvider {
    ...
    public AiSuggestTagsResponseDto suggestTags(AiContextDto context) {
        return AiSuggestTagsResponseDto.builder()
                .tags(List.of(
                        AiSuggestTagsResponseDto.TagSuggestion.builder().name("login-issue").isNew(true)
                                .reason("Mentions login").build(),
                        AiSuggestTagsResponseDto.TagSuggestion.builder().name("urgent").isNew(true)
                                .reason("Stub reason urgent").build()))
                .runId(UUID.randomUUID().toString())
                .provider(getProviderName())
                .model(getModelId())
                .latencyMs(120L)
    ...
    public AiDraftResponseDto draftResponse(AiContextDto context) {
        return AiDraftResponseDto.builder()
                .draft("Hello, \n\nWe have received your request regarding '" + context.getRequestTitle()
